...
const registry = new ProviderRegistry().register(new ChatGPTWebProvider()).register(new DeepSeekWebProvider()).register(new OpenAICompatibleProvider());
const runtime = await new WagentRuntime(registry).init();
const rl = createInterface({ input: stdin, output: stdout });
let model = process.env.WAGENT_MODEL ?? "auto";
let sessionId: string | undefined;
console.log(`Wagent CLI — model=${model}. Commands: /model <id>, /new, /providers, /skills, /quit`);
...
while (true) {
  ...
  if (line === "/skills") { runtime.listSkills().forEach(skill => console.log(`${skill.name} — ${skill.description}`)); continue; }
  try {
